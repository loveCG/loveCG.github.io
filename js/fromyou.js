define([], function (){
	...
	var getPhotoData = function(){
		...
		$.getJSON('fromyou/data.txt').success(function(content){ 
			data = content;
		});
		return data;
	}
	...
	var changeSize = function(){	
		if($(document).width() <= 600){
			...
			var size = Math.max(width*0.20, 105);   
			//var size = Math.max(width*0.26, 157);
			$(".img-box").width(size).height(1700/1080*size);
		}
		$('.instagram img').each(function() {
			
			/* 强制指定大小 */
			$(this).css("height", 1700/1080*size);
			$(this).css("width", size); 
